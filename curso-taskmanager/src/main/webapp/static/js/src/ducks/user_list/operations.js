import * as actions from "es6!src/ducks/user_list/actions";

import { GET, API_URL } from "es6!src/helpers/ajax";

export const fetchUsers = (pageNumber, pageMax) => {
  return dispatch => {
    ...
    GET(API_URL + "/user/init" + page + max)
      .then(response => {
...
export const asyncSearch = query => {
  return dispatch => {
    ...
    GET(API_URL + "/user/search" + searchQuery)
      .then(response => {

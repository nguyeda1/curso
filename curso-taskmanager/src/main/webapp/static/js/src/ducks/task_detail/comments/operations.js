...
import * as actions from "es6!src/ducks/task_detail/comments/actions";

export const fetchListByTaskId = (id, pageNumber, pageMax) => {
  return dispatch => {
    ...
    GET(API_URL + "/comment/task/" + id + page + max)
      .then(response => {
...
export const addItem = (id, data, okCallback) => {
  return dispatch => {
    ...
    POST(API_URL + "/comment/task/" + id, data)
      .then(response => {
        okCallback();
